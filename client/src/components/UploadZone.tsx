...
const UploadZone = ({label, onClear, file, onChange, }: UploadZoneProps) => {
  return (
    <div className='relative group'>
       <div className={`relative h-64 rounded-2xl border-2 border-dashed transition-all duration-300 flex flex-col items-center justify-center bg-white/20 p-6 ${file ? 'border-violet-500/50 bg-violet-500/5' : 'border-white/10 hover:border-violet-500/30 hover:bg-white/5'}`}>
        {file ? (
          <>
            <img src={URL.createObjectURL(file)} alt={label} className='absolute inset-0 w-full h-full object-cover rounded-xl opacity-60'/>
